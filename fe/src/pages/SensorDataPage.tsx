...
const SensorDataPage: React.FC = () => {
  ...
  // Update filtered data when sensor data changes
  React.useEffect(() => {
  ...
  const handleSearch = () => {
    ...
    if (searchTemperature.trim()) {
      ...
      if (!isNaN(temp)) {
        filtered = filtered.filter(item => {
          ...
          return Math.abs(itemTemp - temp) <= 0.1; // Allow small floating point differences
        });
      }
    }
    ...
    if (searchHumidity.trim()) {
      ...
      if (!isNaN(humidity)) {
        filtered = filtered.filter(item => {
          const itemHumidity = item.humidity;
          return Math.abs(itemHumidity - humidity) <= 0.1;
        });
      }
    }
    ...
    if (searchLight.trim()) {
      ...
      if (!isNaN(light)) {
        filtered = filtered.filter(item => {
          ...
          return Math.abs(itemLight - light) <= 0.1;
        });
      }
    }

    // Timestamp filter - case insensitive partial match
  ...
  const columns = [
    {
      header: 'ID',
      accessor: 'id' as keyof SensorData,
    },
    {
      header: 'Nhiệt độ (°C)',
      accessor: 'temperature' as keyof SensorData,
    },
    {
      header: 'Độ ẩm (%)',
      accessor: 'humidity' as keyof SensorData,
    },
    {
      header: 'Ánh sáng (lux)',
      accessor: 'light' as keyof SensorData,
    },
    {
      header: 'Thời gian',
